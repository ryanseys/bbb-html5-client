...
exports.post_chat = function(req, res) {
  ...
  if((username) && (meetingid) && (username.length < max_username_length) && (meetingid.length < max_meetingid_length)) {
	  var userid = "user:" + req.sessionID;
	  store.hmset(userid, "username", username, "meetingID", meetingid, "refreshing", false, "dupSess", false, "sockets", 0);
	  store.sadd("users", userid);
...
exports.get_chat = function(req, res) {
	...
	gfunc.getUserProperty(req.cookies['id'], "username", function(reply) {
	  res.render('chat', { title: 'BigBlueButton HTML5 Chat', user: reply, max_chat_length: max_chat_length });
	});
};
